...
export async function generateMetadata({
  ...
  const ogImage = `${BASE}/api/og?title=${encodeURIComponent(post.title)}&description=${encodeURIComponent(post.description)}&category=${encodeURIComponent(meta.label)}`;
...
export default async function ArticlePage({ params }: { params: Promise<Params> }) {
  ...
  const articleUrl = `${BASE}/articles/${post.slug}`;
  const isNews = post.category === "news" || post.category === "model-release";
  ...
  return (
    <article className="pt-16">
      ...
      {isNews ? (
        <JsonLd type="newsarticle" data={{ headline: post.title, description: post.description, datePublished: post.date, authors: authors.map((a) => ({ name: a.name, url: `${BASE}/authors/${a.slug}` })), url: articleUrl, keywords: post.tags, section: meta.label }} />
      ) : (
        <JsonLd type="article" data={{ headline: post.title, description: post.description, datePublished: post.date, authors: authors.map((a) => ({ name: a.name, url: `${BASE}/authors/${a.slug}` })), url: articleUrl, keywords: post.tags }} />
      )}

      <JsonLd type="breadcrumb" data={{ items: [{ name: "Home", url: BASE }, { name: meta.plural, url: `${BASE}/category/${meta.route}` }, { name: post.title, url: articleUrl }] }} />

      <header className="relative overflow-hidden border-b border-line">
        <CoverArt seed={post.slug} color={meta.color} label={post.slug} className="absolute inset-0 h-full w-full opacity-25" />
        ...
        <div className="relative mx-auto max-w-4xl px-5 pb-16 pt-20 sm:px-8">
          <Reveal>
            ...
            <h1 className="display mt-6 text-4xl leading-[1.08] text-bone sm:text-5xl">{post.title}</h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[0.68rem] tracking-[0.16em] uppercase text-dim">
              {authors.map((a) => (
                <Link key={a.slug} href={`/authors/${a.slug}`} className="text-fog transition-colors hover:text-signal">{a.name}</Link>
              ))}
          ...
          <footer className="mt-16 border-t border-line pt-10">
            {authors.length > 0 && (
              <div className="mb-12 flex flex-col gap-6 border border-line bg-panel p-6 sm:flex-row sm:items-center">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center border font-mono text-lg" style={{ borderColor: `${meta.color}55`, color: meta.color }} aria-hidden>
                  {authors[0].name.slice(0, 1)}
                </div>
                <div>
                  <p className="font-mono text-[0.62rem] tracking-[0.2em] uppercase text-dim">Filed by</p>
                  <p className="display mt-1 text-lg text-bone">
                    <Link href={`/authors/${authors[0].slug}`} className="hover:text-signal">{authors[0].name}</Link>
                  </p>
